...
def main_page(location,term) #this takes in a search url and return a hash of links page names and postdates for that locaton
  job_master={}
  location.each do |p|
    open_search_page=Nokogiri::HTML(open("http://#{p}.craigslist.org/search/jjj?query=#{term}")).css("a")
    for x in open_search_page
      if (x.attr("href")[0]=="/")&&(x.attr("href")[-1]=="l")&&(x.attr("class")!="i")
          job_master["http://#{p}.craigslist.org#{x.attr("href")}"]={"time"=>get_post_date("http://#{p}.craigslist.org#{x.attr("href")}"),"text"=>x.text, "location"=>p}
      end
    end
  end
  return job_master
